...
// INT4 groupwise dequant GEMV (CSCD F0 stage from cascade/kernels/int4.py):
// codes pack 2 weights/byte (even column in the LOW nibble), nibbles are
// two's-complement in [-8, 7], one FP32 scale per (row, group).
// Y[r] = sum_g scales[r*n_groups+g] * sum_{c in group g, c < cols} q(r,c) * X[c]
