...
# Faixas pinadas (piso testado + teto de compatibilidade). Nunca `-U` sem pino.
PINNED_PIP_SPECS = {
    "xxhash": "xxhash>=3.4,<4",
    "sentencepiece": "sentencepiece>=0.1.99,<1",
    "tiktoken": "tiktoken>=0.7,<1",
    "transformers": "transformers>=4.56,<5",
...
def _colab_importable() -> bool:
    """Colab de verdade: google.colab importável (não apenas env vars)."""
    try:
...
def ensure_import(module: str, pip_name: str | None = None):
    try:
        ...
        pkg = pip_name or PINNED_PIP_SPECS.get(module, module)
        if not _auto_install_allowed():
            raise SystemExit(
                f"Dependência ausente: {module}. Instalação automática desativada "
                "fora do Colab (defina RIFT_AUTO_INSTALL=1 para permitir) — "
...
def ensure_phase1_dependencies():
    ...
    if _auto_install_allowed():
        ensure_import("sentencepiece")
        ensure_import("tiktoken")
        print("[deps] Garantindo transformers/accelerate/huggingface_hub nas faixas pinadas...")
        subprocess.check_call(
            [
                ...
                "install",
                "-q",
                PINNED_PIP_SPECS["transformers"],
                PINNED_PIP_SPECS["accelerate"],
                PINNED_PIP_SPECS["huggingface_hub"],
            ]
        )
    else:
        print(
            "[deps] Instalação automática desativada (fora do Colab e sem "
            "RIFT_AUTO_INSTALL=1); usando os pacotes já instalados."
        )
...
def _read_vmrss_bytes() -> int | None:
    """VmRSS do processo via /proc/self/status (Linux/Colab); None fora do Linux."""
    try:
...
def _getrusage_peak_bytes() -> int | None:
    """Pico de RSS do processo (ru_maxrss em KB no Linux); None se indisponível."""
    try:
        import resource
        peak_kb = resource.getrusage(resource.RUSAGE_SELF).ru_maxrss
    except Exception:
        return None
    if peak_kb and peak_kb > 0:
        return int(peak_kb) * 1024
    return None


class RamPhaseSampler:
    """Thread que amostra VmRSS (~1ms) durante uma fase de benchmark.

    Uso: ``with RamPhaseSampler() as sampler: <fase>`` e depois
    ``sampler.result()``. Métodos, em ordem de preferência:
      1. ``proc_vmrss_sampling_per_phase_v1`` — /proc/self/status a ~1ms
         (máximo e média por fase);
      2. ``getrusage_peak_fallback`` — resource.getrusage(RUSAGE_SELF).ru_maxrss
         (pico do processo, não da fase; KB no Linux);
      3. ``None`` — sem medição real; os campos de RAM ficam null
         (nunca inventamos RAM).
    """
    ...
    def __enter__(self) -> "RamPhaseSampler":
    ...
    def result(self) -> Dict[str, Any]:
        ...
        return {"max_bytes": None, "mean_bytes": None, "samples": 0, "method": None}
...
def build_schema_v2_fields(
    *,
    model_id: str,
    device: str | None,
    benchmark_protocol: str = "LINEAR_REFERENCE_V2",
    implementation_kind: str = "REFERENCE_MEASURED",
) -> Dict[str, Any]:
    ...
    group_key = f"{benchmark_protocol}|{model_id}|{device_str}|{torch_version}"
    return {
        "schema_version": 2,
        ...
        "implementation": {
            ...
            "native": implementation_kind == "NATIVE_MEASURED",
            "simulated": implementation_kind == "SIMULATED",
        },
    }
    ...
    def record(
        ...
        *,
        ...
        record = {
            ...
            # Schema v2 (docs/C3_CONTRACTS_V1.md §3) — evolução append-only;
            # os aliases legados rift_*/baseline_* permanecem intactos.
            **build_schema_v2_fields(
                model_id=self.model_id,
                device=self.device,
...
def resolve_torch_device(requested: str):
    """auto/cuda: GPU se disponível; senão CPU (Colab sem GPU / TPU sem CUDA)."""
    import torch
...
def cleanup_colab_workspace(*, label: str = "battery", wipe_hf_cache: bool = False) -> None:
    ...
    # Guarda de segurança: limpeza destrutiva só no Colab (google.colab
    # importável) ou com opt-in explícito RIFT_ALLOW_LOCAL_CLEANUP=1 — e, fora
    # do Colab, apenas em caminhos sob /tmp ou /content. Nunca apaga diretórios
    # de uma máquina de desenvolvimento local (Windows/Linux) por engano.
    in_colab = _colab_importable()
    allow_local = os.environ.get("RIFT_ALLOW_LOCAL_CLEANUP", "").strip() == "1"
    if not in_colab and not allow_local:
        print(
            f"[cleanup] {label}: execução local fora do Colab — limpeza "
            "destrutiva ignorada (defina RIFT_ALLOW_LOCAL_CLEANUP=1 para permitir)."
        )
        gc.collect()
        return
    ...
    def _target_allowed(path: Path) -> bool:
        if in_colab:
            return True
        posix = path.as_posix()
        return posix.startswith("/tmp/") or posix.startswith("/content/")
...
# ------------------------------------------------------------------------------
# 9.5 E2E tok/s de modelo completo — P1_RIFT_E2E_TOKS
...
E2E_MEASUREMENT_SCOPE = (
    ...
    "de referência do codec RIFT (BASE 2-bit + REFINEMENT 2-bit progressivo, "
    "W0 dequantizado fp32 cacheado, W denso original fora do caminho quente) "
    "— runtime de referência Python — velocidade não representa kernel "
    "nativo. RAM=pico VmRSS por fase; disco=payloads F0/F1 reais (os.stat)."
)


def _e2e_max_params() -> float:
...
def _e2e_find_transformer_blocks(model) -> List[Tuple[str, Any]]:
...
def _e2e_set_module_by_path(root, dotted: str, new_module) -> None:
...
def _e2e_collect_linears(block, block_name: str) -> Dict[str, Any]:
...
_RIFT_INLINE_LINEAR_CLS = None


def _make_rift_inline_linear_cls():
    """Cria (uma única vez) a classe do módulo inline; torch só existe em runtime."""
    global _RIFT_INLINE_LINEAR_CLS
    if _RIFT_INLINE_LINEAR_CLS is not None:
        return _RIFT_INLINE_LINEAR_CLS

    class RiftInlineLinearModule(torch.nn.Module):
        """Substitui nn.Linear pelo runtime de referência do codec RIFT.

        Crib da técnica C3InlineLinearModule/CascadeLinearModule: quantiza W
        UMA vez (Q4_LINEAR_TEST: BASE 2-bit + REFINEMENT 2-bit progressivo,
        reutilizando pack_int2/quantize/decode deste script), cacheia W0
        dequantizado em fp32 e mantém o W denso original FORA do caminho
        quente (apenas streams packed + W0 residem no módulo). Runtime de
        referência Python — velocidade não representa kernel nativo.
        """

        def __init__(self, linear):
            super().__init__()
            weight = linear.weight.detach().float().cpu().contiguous()
            codec = quantize_q4_linear_test(weight.numpy())
            self.out_features = int(weight.shape[0])
            self.in_features = int(weight.shape[1])
            self.value_count = int(codec["value_count"])
            self.w_min = float(codec["w_min"])
            self.w_max = float(codec["w_max"])
            self.base_packed = codec["base_packed"]
            self.ref_packed = codec["ref_packed"]
            device = linear.weight.device
            w0 = decode_q4_linear_test(codec, use_refinement=True)
            self._w0 = torch.from_numpy(w0).to(device=device, dtype=torch.float32)
            self._bias = (
        ...
        def forward(self, x):
            ...
            y = F.linear(x2, self._w0, self._bias)
            if y.dtype != x.dtype:
                y = y.to(dtype=x.dtype)
            return y.reshape(*orig_shape[:-1], self.out_features)

    _RIFT_INLINE_LINEAR_CLS = RiftInlineLinearModule
    return _RIFT_INLINE_LINEAR_CLS


def _e2e_restore_patched(patched: Dict[str, Tuple[Any, str, Any]]) -> None:
    """Devolve as nn.Linear originais aos blocos (rollback/limpeza final)."""
    for full, (block, short, original) in list(patched.items()):
        try:
            _e2e_set_module_by_path(block, short, original)
        except Exception as exc:
            print(f"[E2E] AVISO: rollback de {full} falhou: {exc}")


def _e2e_patch_all_blocks(
    model,
    ...
    Grava payloads F0 (BASE 2-bit) e F1 (REFINEMENT 2-bit) reais por Linear em
    <out>/artifacts/e2e/ e devolve (patched, baseline_disk_bytes,
    ...
    """
    inline_cls = _make_rift_inline_linear_cls()
    ...
    try:
        for block_name, block in blocks:
            for full, linear in _e2e_collect_linears(block, block_name).items():
                module = inline_cls(linear)
                slug = _safe_slug(full)
                header = struct.pack(
                    "<4sffQ", b"Q4L0", module.w_min, module.w_max, module.value_count
                )
                f0_path = artifacts_dir / f"{slug}.f0.bin"
                f1_path = artifacts_dir / f"{slug}.f1.bin"
                f0_path.write_bytes(header + module.base_packed)
                f1_path.write_bytes(header + module.ref_packed)
                ...
                _e2e_set_module_by_path(block, short, module)
                patched[full] = (block, short, linear)
    except Exception:
        _e2e_restore_patched(patched)
        patched.clear()
        raise
    if not patched:
...
def _e2e_forward_logits(model, inputs: Dict[str, Any]):
...
def _token_exact_match(a: List[int], b: List[int]) -> Dict[str, Any]:
    n = min(len(a), len(b))
    if n == 0:
        return {
...
def run_e2e_toks_battery(
    *,
    ...
    """P1_RIFT_E2E_TOKS: baseline E candidato REAIS via model.generate.
    ...
    """
    battery_id = "P1_RIFT_E2E_TOKS"
    print("\n" + "=" * 78)
    print("P1_RIFT_E2E_TOKS — tok/s de modelo completo (contracts §12)")
    print("=" * 78)
    ...
    blocks = _e2e_find_transformer_blocks(model)
    if not blocks:
        recorder.record(
            battery_id=battery_id,
            status="FAIL",
    ...
    for block_name, block in blocks:
        for linear in _e2e_collect_linears(block, block_name).values():
            block_linear_params += int(linear.weight.numel())
    w0_cache_bytes = block_linear_params * 4       # W0 fp32 dequantizado cacheado
    packed_bytes_est = block_linear_params // 2    # BASE 2-bit + REF 2-bit
    ...
    if device.type == "cuda":
        ...
        if free_vram is not None and w0_cache_bytes > 0.8 * free_vram:
            return _skip(
                "VRAM insuficiente para o cache fp32 do W0 dequantizado",
                {
        ...
        if mem_available is not None and (w0_cache_bytes + packed_bytes_est) > 0.8 * mem_available:
            return _skip(
                "RAM insuficiente para o cache fp32 do W0 dequantizado (CPU)",
                {
                    "w0_cache_bytes": w0_cache_bytes,
    ...
    try:
        ...
        logits_base = _e2e_forward_logits(model, enc)
        with RamPhaseSampler() as baseline_sampler:
            base_gen = _measure_generate_tok_s(
                model, tokenizer, E2E_GENERATION_PROMPT, device,
                max_new_tokens=E2E_MAX_NEW_TOKENS, warmup=2, timed=3,
            )
        baseline_phase = baseline_sampler.result()
        print(
        ...
        print(
            ...
            "(BASE 2-bit + REFINEMENT 2-bit)..."
        )
        with RamPhaseSampler() as patch_sampler:
            patched, baseline_disk, candidate_disk = _e2e_patch_all_blocks(
                model, blocks, artifacts_dir
            )
        patch_phase = patch_sampler.result()
        n_linears = len(patched)
        print(f"[E2E] {n_linears} nn.Linear substituídas; artefatos F0/F1 em {artifacts_dir}")

        logits_cand = _e2e_forward_logits(model, enc)
        with RamPhaseSampler() as candidate_sampler:
            cand_gen = _measure_generate_tok_s(
                model, tokenizer, E2E_GENERATION_PROMPT, device,
                max_new_tokens=E2E_MAX_NEW_TOKENS, warmup=2, timed=3,
            )
        candidate_phase = candidate_sampler.result()
        print(f"[E2E] candidato {cand_gen['tok_s_median']:.2f} tok/s")

        em = _token_exact_match(base_gen["new_token_ids"], cand_gen["new_token_ids"])
        logits_q = None
        if logits_base is not None and logits_cand is not None:
            logits_q = compute_metrics(logits_base.numpy(), logits_cand.numpy())
        ...
        ram_method = baseline_phase.get("method") or candidate_phase.get("method")

        recorder.record(
            battery_id=battery_id,
            status="PASS" if e2e_pass else "FAIL",
            baseline_tok_s=baseline_tok_s,
            rift_tok_s=candidate_tok_s,
            baseline_ram_bytes=baseline_phase.get("max_bytes"),
            rift_ram_bytes=candidate_phase.get("max_bytes"),
            baseline_disk_bytes=int(baseline_disk),
            rift_disk_bytes=int(candidate_disk),
            measurement_scope=E2E_MEASUREMENT_SCOPE,
            quality={
            ...
            metrics={
                "e2e": {
                    ...
                    "codec": "Q4_LINEAR_TEST progressivo (BASE 2-bit + REFINEMENT 2-bit)",
                    "runtime": "python_reference_rift_codec",
                    "w0_cache_fp32": True,
...
def run_phase1(
    *,
    ...
    try:
        ...
        try:
            ...
            fail_record = {
                ...
                **build_schema_v2_fields(model_id=model_id, device=str(device)),
            }
    ...
    # -------------------------
    # RAM real por fase: thread de amostragem de VmRSS (~1ms) envolvendo cada
    # fase de benchmark (baseline vs candidato) — ver docs/C3_CONTRACTS_V1.md §3.
    with RamPhaseSampler() as baseline_ram_sampler:
        baseline_perf = benchmark_ms(
            lambda: F.linear(x2d, W_ref_t),
            device=device,
            iterations=iterations,
        )
    baseline_phase_ram = baseline_ram_sampler.result()
    ...
    # ------------------------------------------------------------------
    # Campos *_ram_bytes de nível superior: SOMENTE RSS medido (pico de VmRSS
    # por fase, baseline vs candidato). Sem medição (ex.: Windows local) → null.
    #
    # As estimativas aritméticas de working set abaixo continuam sendo úteis
    # como diagnóstico, mas vivem apenas em metrics.memory.estimated_*:
    #   baseline = W_FP32 + X + Y
    #   RIFT     = packed streams + W_reconstructed_FP32 + X + Y
    # No protótipo de referência o peso RIFT é reconstruído para FP32 antes
    # da GEMM; o futuro kernel low-bit fused deverá reduzir esse valor.
    baseline_ram_measured = baseline_phase_ram["max_bytes"]
    base_ram_measured = base_phase_ram["max_bytes"]
    full_ram_measured = full_phase_ram["max_bytes"]
    ram_method = (
        baseline_phase_ram["method"]
        or base_phase_ram["method"]
        or full_phase_ram["method"]
    )
    ...
    recorder.record(
        ...
        baseline_tok_s=None,
        rift_tok_s=None,
        baseline_ram_bytes=baseline_ram_measured,
        rift_ram_bytes=base_ram_measured,
        baseline_disk_bytes=source_tensor_bytes,
        rift_disk_bytes=int(payload_sizes["base_payload_disk_bytes"]),
        measurement_scope=(
            "Single Linear op. RAM=measured per-phase process RSS peak "
            "(VmRSS sampling) when available, else null; disk=real bytes on "
            "disk; Tok/s=model-level NOT_MEASURED."
        ),
        quality={
        ...
        metrics={
            ...
            "memory": {
                "method": ram_method,
                "baseline_phase": baseline_phase_ram,
                "candidate_phase": base_phase_ram,
                "estimated_baseline_bytes": baseline_ram_working_set,
                "estimated_candidate_bytes": base_ram_working_set,
                "estimated_includes_reconstructed_fp32_weight": True,
            },
        },
        notes=(
    ...
    recorder.record(
        ...
        rift_tok_s=None,
        baseline_ram_bytes=baseline_ram_measured,
        rift_ram_bytes=full_ram_measured,
        baseline_disk_bytes=source_tensor_bytes,
        ...
        metrics={
            ...
            "memory": {
                "method": ram_method,
                "baseline_phase": baseline_phase_ram,
                "candidate_phase": full_phase_ram,
                "estimated_baseline_bytes": baseline_ram_working_set,
                "estimated_candidate_bytes": full_ram_working_set,
                "estimated_includes_reconstructed_fp32_weight": True,
            },
        },
        notes=(
    ...
    # ------------------------------------------------------------------
    # P1_RIFT_E2E_TOKS — tok/s de modelo completo (docs/C3_CONTRACTS_V1.md §12)
    # ------------------------------------------------------------------
    try:
        ...
        print(f"[E2E] AVISO: bateria P1_RIFT_E2E_TOKS abortada: {e2e_exc}")
        e2e_summary = {
            "battery_id": "P1_RIFT_E2E_TOKS",
            "status": "ERROR",
    ...
    report = {
        ...
        "memory": {
            "measurement_scope": "per_phase_process_rss_sampling",
            "method": ram_method,
            "baseline_ram_bytes": baseline_ram_measured,
            "rift_base_ram_bytes": base_ram_measured,
            "rift_full_ram_bytes": full_ram_measured,
            "phases": {
                "baseline_fp32_matmul": baseline_phase_ram,
                "rift_base_decode_plus_matmul": base_phase_ram,
                "rift_full_decode_plus_matmul": full_phase_ram,
            },
            "estimated": {
                "baseline_working_set_bytes": baseline_ram_working_set,
                "rift_base_working_set_bytes": base_ram_working_set,
                "rift_full_working_set_bytes": full_ram_working_set,
...
def main(argv: Iterable[str] | None = None):
    ...
    parser.add_argument(
        "--device",
        default="auto",
        help="auto (cuda se disponível, senão cpu), cuda ou cpu",
    )
    parser.add_argument(
        "--iterations",
        type=int,
