...
def google_colab_importable() -> bool:
    try:
...
def pip_auto_install_allowed() -> bool:
    """pip install automático apenas no Colab ou com RIFT_AUTO_INSTALL=1."""
    if os.environ.get("RIFT_AUTO_INSTALL", "").strip() == "1":
        return True
    return google_colab_importable()
...
def ensure_import(module: str, pip_name: str | None = None):
    try:
        ...
        if not pip_auto_install_allowed():
            raise SystemExit(
                f"Dependência ausente: {module}. Instalação automática desativada fora do Colab. "
                f'Instale manualmente (pip install "{package}") ou defina RIFT_AUTO_INSTALL=1.'
            )
...
xxhash = ensure_import("xxhash", "xxhash>=3.0,<4")
torch = None
F = None
...
def ensure_ml_dependencies() -> None:
    ...
    if torch is not None:
        return
    ensure_import("sentencepiece", "sentencepiece>=0.1.99,<1")
    ensure_import("tiktoken", "tiktoken>=0.7,<1")
    if pip_auto_install_allowed():
        print("[deps] Garantindo transformers/accelerate/huggingface_hub em faixas pinadas (Gemma 4 / multimodal)...")
        subprocess.check_call(
            [sys.executable, "-m", "pip", "install", "-q", *PINNED_ML_REQUIREMENTS]
        )
    else:
        print("[deps] pip automático desativado (fora do Colab e sem RIFT_AUTO_INSTALL=1); usando versões já instaladas.")
    try:
...
class CascadeFormatError(ValueError):
    """Formato CSCD inválido/corrompido (header, IR, stage pages ou payload)."""
...
RAM_METHOD_VMRSS = "proc_vmrss_sampling_per_phase_v1"
RAM_METHOD_GETRUSAGE = "getrusage_peak_fallback"


def _read_vmrss_bytes() -> int | None:
    """VmRSS (bytes) de /proc/self/status; None fora do Linux."""
    try:
...
def _getrusage_peak_bytes() -> int | None:
    """Pico de RSS do processo via resource.getrusage (KB no Linux); None se indisponível."""
    try:
        import resource
...
class RamSampler:
    """Thread que amostra VmRSS a ~1ms durante uma fase de benchmark."""
    ...
    def __enter__(self) -> "RamSampler":
        first = _read_vmrss_bytes()
...
def measure_phase_ram(fn):
    """Executa fn amostrando VmRSS; retorna (resultado, medição da fase)."""
    with RamSampler() as sampler:
        result = fn()
    if sampler.samples:
        phase = {
            "method": RAM_METHOD_VMRSS,
            "max_bytes": int(max(sampler.samples)),
            "mean_bytes": float(sum(sampler.samples) / len(sampler.samples)),
            "samples": len(sampler.samples),
        }
    else:
        peak = _getrusage_peak_bytes()
        if peak is not None:
            phase = {"method": RAM_METHOD_GETRUSAGE, "max_bytes": peak, "mean_bytes": None, "samples": 0}
        else:
            phase = {"method": None, "max_bytes": None, "mean_bytes": None, "samples": 0}
    return result, phase


def measured_phase_max_vmrss(phase: dict[str, Any]) -> int | None:
    """Top-level *_ram_bytes: apenas máximo VmRSS medido por fase; senão None."""
    if phase.get("method") == RAM_METHOD_VMRSS:
        return phase.get("max_bytes")
    return None
...
def memory_metrics(
    ...
    *,
    estimated_baseline_bytes: int | None,
    estimated_candidate_bytes: int | None,
) -> dict[str, Any]:
    """Bloco metrics.memory: medições por fase + estimativas aritméticas (nunca top-level)."""
    return {
        "method": candidate_phase.get("method") or baseline_phase.get("method"),
...
def build_comparison_context(*, device: str) -> dict[str, Any]:
    torch_version = None
    try:
        if torch is not None:
    ...
    try:
        ...
        if module is None:
            import transformers as module  # type: ignore[no-redef]
        transformers_version = str(getattr(module, "__version__", "") or "") or None
    except Exception:
        transformers_version = None
    return {
...
def build_comparison_group_id(model_id: str, context: dict[str, Any]) -> str:
    basis = "|".join([
        str(context.get("protocol")),
        str(model_id),
        str(context.get("device")),
        str(context.get("torch")),
    ])
    return "cmp-" + hashlib.sha256(basis.encode("utf-8")).hexdigest()[:24]


class BatteryRecorder:
    def __init__(self, out_dir: Path, *, model_id: str, publish_mode: str = "off", results_endpoint: str | None = None, device: str = "unknown"):
        ...
        self.comparison_context = build_comparison_context(device=self.device)
        self.comparison_group_id = build_comparison_group_id(self.model_id, self.comparison_context)
    ...
    def record(
        ...
        *,
        ...
        simulated = battery_id == "P1_CASCADE_PREDICTIVE_PREFETCH_SIM" or status == "SIMULATED"
        record = {
        ...
        if simulated:
            record["eligible_for_primary_ranking"] = False
...
def cleanup_colab_workspace(*, label: str = "battery", wipe_hf_cache: bool = False) -> None:
    ...
    colab_env = google_colab_importable()
    allow_local = os.environ.get("RIFT_ALLOW_LOCAL_CLEANUP", "").strip() == "1"

    def _removal_allowed(path: Path) -> bool:
        # Guarda destrutiva: fora do Colab só remove caminhos sob /content ou /tmp;
        # qualquer outro caminho local exige RIFT_ALLOW_LOCAL_CLEANUP=1.
        if colab_env or allow_local:
            return True
        posix = path.as_posix()
        return (
            posix == "/content" or posix.startswith("/content/")
            or posix == "/tmp" or posix.startswith("/tmp/")
        )
    ...
    if wipe_hf_cache:
        ...
        for path in targets:
            if not _removal_allowed(path):
                skipped_guard.append(str(path))
                continue
            try:
                if path.is_dir():
    ...
    for pattern in patterns:
        for match in _glob.glob(pattern):
            p = Path(match)
            if not _removal_allowed(p):
                skipped_guard.append(str(p))
                continue
            try:
                if p.is_dir():
    ...
    if skipped_guard:
        print(
            f"[cleanup] {label}: {len(skipped_guard)} caminho(s) preservado(s) fora do Colab "
            "(defina RIFT_ALLOW_LOCAL_CLEANUP=1 para liberar a remoção local)."
        )
    if removed:
...
def run_phase1(args: argparse.Namespace) -> Path:
    ...
    baseline_perf, baseline_ram_phase = measure_phase_ram(
        lambda: benchmark_ms(lambda: F.linear(x, W), device=device, iterations=args.iterations)
    )
    f0_perf, f0_ram_phase = measure_phase_ram(
        lambda: benchmark_ms(lambda: lowrank_linear(x, U0, S0, V0), device=device, iterations=args.iterations)
    )
    gated_perf, gated_ram_phase = measure_phase_ram(
        lambda: benchmark_ms(cascade_reference, device=device, iterations=args.iterations)
    )
    ...
    # Estimativas aritméticas de working-set: ficam SOMENTE em metrics.memory.estimated_* (nunca top-level).
    estimated_baseline_ram = baseline_disk + input_bytes + output_bytes
    estimated_f0_ram = stage0_bytes + input_bytes + output_bytes
    estimated_candidate_ram = factor_bytes + input_bytes + output_bytes
    # Top-level *_ram_bytes: apenas RSS medido por fase (máx. VmRSS); sem medição -> None.
    baseline_ram = measured_phase_max_vmrss(baseline_ram_phase)
    f0_ram = measured_phase_max_vmrss(f0_ram_phase)
    candidate_ram = measured_phase_max_vmrss(gated_ram_phase)
    ...
    recorder.record(
        ...
        candidate_ram_bytes=f0_ram,
        baseline_disk_bytes=baseline_disk,
        candidate_disk_bytes=int(bundle["stages"][0]["size"]),
        measurement_scope="Single Linear op; disk=FP32 raw vs F0 FP16 low-rank payload; RAM=measured process RSS per phase (VmRSS sampling) when available, working-set estimate em metrics.memory.estimated_*; model Tok/s not measured.",
        quality={"full_local_gate_pass": None, "output": q_f0},
        metrics={
            "operation": {"baseline_median_ms": baseline_perf["median_ms"], "candidate_median_ms": f0_perf["median_ms"], "speedup_x": baseline_perf["median_ms"] / f0_perf["median_ms"]},
            "memory": memory_metrics(
                baseline_ram_phase,
                f0_ram_phase,
                estimated_baseline_bytes=estimated_baseline_ram,
                estimated_candidate_bytes=estimated_f0_ram,
            ),
        },
    ...
    recorder.record(
        ...
        measurement_scope="Single Linear op; disk=arquivo CSCD real; RAM=measured process RSS per phase (VmRSS sampling) when available, working-set estimate em metrics.memory.estimated_*; latency=Python/PyTorch reference path; model Tok/s not measured.",
        quality={
...
def main(argv: Iterable[str] | None = None) -> None:
    ...
    parser.add_argument("--device", default="auto", help="auto (cuda se disponível, senão cpu), cpu ou cuda")
