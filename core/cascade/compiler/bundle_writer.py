...
def write_cascade_bundle(
    ...
    *,
    ...
    f0_codec: str = "INT4_GROUP",
) -> Dict[str, Any]:
    """Escreve model.cascade mínimo com F0 (codec declarado) + F1 low-rank.

    `f0_codec` declara o codec REAL dos payloads de stage 0 no stage.meta
    (contrato §6: "Bundles gravam stage.meta.codec"). Default INT4_GROUP
    preserva o comportamento CASCADE; a série C3 passa INT2_GROUP (rift)
    ou TERNARY_ROWSCALE (aether/spectra) quando os codes/scales vêm desses
    codecs — sem isso um leitor decodificaria stage 0 como INT4 e leria lixo.
    """
    path = Path(path)
    ...
    return {
        ...
        "checksum": checksum,
        "stages": [
            {"stage_id": 0, "offset": offsets[0][0], "size": offsets[0][1], "type": "BASE_STAGE", "codec": str(f0_codec)},
