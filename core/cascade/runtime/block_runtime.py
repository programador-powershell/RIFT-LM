...
class CascadeLinearModule(nn.Module):
    ...
    def __init__(self, stages, *, gate_percentile: float = 70.0, path: str = "F0_GATE_F1",
        ...
        # low_mem: dequant por chamada, sem cache denso F0 (opt-in via CASCADE_LOW_MEM=1)
        if low_mem is None:
    ...
    def _w0(self, device: torch.device, dtype: torch.dtype) -> torch.Tensor:
        if self.low_mem:
            w0 = self._dequant_w0(device)
            return w0.to(dtype=dtype) if dtype != torch.float32 else w0
    ...
    def stats(self) -> Dict[str, Any]:
        ...
        # o cache denso FP32 de F0 também é memória residente quando existe
        w0_cache_bytes = (
