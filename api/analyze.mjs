...
function validatePayload(value) {
  ...
  return value.models.map((entry, index) => {
    ...
    return { model_id: modelId, technologies: { RIFT: rift, CASCADE: cascade } };
  });
}
...
function buildRanking(models) {
  ...
  for (const model of models) {
    for (const technology of [model.technologies.RIFT, model.technologies.CASCADE]) {
    ...
    analyses: {
      ...
      items: {
        ...
        properties: {
          ...
          recommendation: { type: "string", enum: ["RIFT", "CASCADE", "INCONCLUSIVO"] },
...
function buildPrompt(models, ranking) {
  ...
  return [
    ...
    "Para cada model_id, recomende RIFT, CASCADE ou INCONCLUSIVO.",
    "Se uma das duas tecnologias estiver ausente, a recomendacao deve ser INCONCLUSIVO.",
...
function validateGeminiAnalysis(value, models) {
  ...
  const analyses = models.map((model) => {
    ...
    let recommendation = ["RIFT", "CASCADE", "INCONCLUSIVO"].includes(source.recommendation)
      ? source.recommendation
      : "INCONCLUSIVO";
    if (!model.technologies.RIFT || !model.technologies.CASCADE) recommendation = "INCONCLUSIVO";
    return {
