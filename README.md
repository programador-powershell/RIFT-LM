...
- **Entrada GGUF no conversor + escada de codecs por tensor** (contrato §27): `GGUFSource` lê `.gguf` do llama.cpp em **streaming por blocos de linhas** (o ggml não deixa um bloco cruzar a fronteira da linha), então o pico de RAM fica em `chunk_rows × colunas × 4 B` em vez do tensor inteiro; novo codec `INT2_GROUP_ASYMMETRIC_MINMAX` (4 códigos/byte, `w = q·scale + min`, 2 + 32/grupo bpw) ao lado do INT4; escada `--codec-ladder` tenta do degrau mais barato ao mais caro e fica no **primeiro que passa o gate** — o gate é o MESMO em todos os degraus, então a escada reduz bytes sem afrouxar qualidade, e se nenhum passa o tensor vai para passthrough exato. Requer `pip install 'gguf>=0.10,<1'` (**sujeito a homologação TI/SI**).
- **Orçamento de RAM derivado da máquina + RAM por largura de bits** (contrato §28): `--target-ram-gb 0` (novo padrão) calcula o alvo como `total − 8 GiB` com piso de metade da RAM (16 GB → 8, 24 GB → 16, 32 GB → 24) via `/proc/meminfo → os.sysconf → GlobalMemoryStatusEx`, **sem dependência nova**; `--ram-budget-mb 0` escala a fatia de conversão junto (`clamp(total/512, 16 MB, 128 MB)`); `summary.residency.memory_by_bits` imprime a tabela de RAM para 1/2/3/4/5/6/8 bits e fp16 no estilo cartão de modelo do Hugging Face, rotulada `PROJETADO` (recalcula só o estágio base e soma o passthrough exato REAL) com a ressalva medida de que 1–2 bits uniformes NÃO preservam qualidade neste projeto (PPL 41,5M em ternário e 22k em INT2 contra 49,1 do original).
- **Card "Modelos convertidos" no painel** (contrato §28.3): lê `battery_id=CASCADE_MODEL_CONVERSION` + `metrics.converter` e mostra residente/paginável/passthrough, bpw médio do F0, degraus escolhidos, veredito contra o orçamento da máquina e a tabela por bits com barras verde/vermelho.
- **Escada automática pelo tipo da fonte** (contrato §29.1): `--codec-ladder auto` é o novo padrão — fonte já low-bit (`GGUF_IQ2`/`Q4_K`/…) usa `safe` e **não tenta INT2**; BF16/F16/F32 usa `compact`. Medido no Muse Glimmer (fonte IQ2): INT2/g64 nunca passou o gate (cosine 0,91–0,92) e o raw custa 2,66 bpw, não 16 — tentar aquele degrau ali era só custo de CPU.
- **Guarda de expansão de bytes** (contrato §29.2, ligada por padrão): antes de gravar cada F0 o conversor projeta os bytes e, se o degrau ficaria ≥ à fonte, pula direto para o passthrough exato (`ladder.stopped_by="byte_expansion_guard"`) — a checagem se repete no total F0+F1. Medido em `o_proj` 256×512 com fonte a 2,66 bpw: **43 581 B exatos** contra **69 632 B com perda** se a guarda for desligada com `--allow-byte-expansion`.
- **Passthrough sem cópia** (contrato §29.3): `--keep-source-passthrough` registra os tensores fora do CASCADE (embeddings, `lm_head`, MoE, fora do `--include-regex`) como `SOURCE_EXTERNAL` (`files:{}`, `bytes:0`, `external_bytes` com o tamanho real) em vez de copiá-los byte a byte — elimina o pico de RAM/disco que estourava em modelo grande. Economiza DISCO, **não** RAM de execução (`external_bytes` continua no residente HOT) e marca `residency.bundle_requires_source`: o bundle deixa de ser autocontido. Exposto no caminho do Colab como `keep_source=on|off` na rota `/converter/<modelo>` e como checkbox no card do conversor (desligado por padrão).
- **Piso de energia do F1 derivado do gate** (contrato §29.4, sem constante mágica): antes de avaliar os ranks o conversor compara a energia do resíduo capturada com o mínimo que o gate exigiria — `max(1 − (nrmse_max/nrmse_f0)², 1 − (1−cosine_min)/(1−cosine_f0))` — e aborta o F1 abaixo de 75% desse mínimo. Medido: o resíduo INT4 entrega `captured_fraction ≈ 0,247` com rank ≤ 32 enquanto `attn_output` precisaria de **0,375** para sair de cosine 0,992 e fechar 0,995, o que explica o rank 8→32 não mover a métrica. `--f1-min-energy` (padrão 0) é piso absoluto opcional por cima.

### :pushpin: Fixes

- `python cascade_converter.py convert --help` levantava `TypeError: %d format: a real number is required, not dict`: o argparse interpola o help com `%`-formatting e um `%` literal quebra a montagem — corrigido com `%%` e coberto por varredura de todos os blocos `add_argument` dos CLIs Python no smoke.
- O relatório mentia sobre a escada com `--codec-ladder auto`: imprimia a escada `compact` mesmo quando todos os tensores usaram `safe`. Agora `summary.codec_ladder` guarda o modo PEDIDO, `summary.codec_ladder_resolved` conta os modos realmente usados e `ladder_rungs` segue o dominante (§29.6).
- `copy_raw_tensor` de entrada GGUF materializava o tensor inteiro com `.tobytes()` (OOM em embedding grande) — passou a gravar em blocos de 8 MB.
- Guarda de expansão também no total F0+F1 (`byte_expansion_with_f1`): sem ela um tensor podia passar o gate e ainda assim ocupar mais bytes que a fonte exata.

### :construction_worker: Refactors

- `--keep-source-passthrough`, `--codec-ladder`, `--include-regex` e `--target-ram-gb` repassados pelo runner local (`/converter.py`) e pela célula do Colab — as flags deixam de existir só no CLI direto.
- `estimate_tensor_output_peak(..., keep_source=True)` projeta 0 bytes para o passthrough sem cópia e `verify_tensor_outputs` valida a existência do checkpoint de origem em vez do arquivo de estágio.
- `randomized_residual_factors` passou a acumular a energia do resíduo em uma única passada (retorna `(u, v, info)` com `captured_fraction` e os maiores valores singulares) — a decisão do F1 fica auditável no manifesto (`attempts[].f1_spectrum`).
