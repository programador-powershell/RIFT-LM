...
- **Registro do conversor publicava ganho que não existe** (contrato §29.9): com `SOURCE_EXTERNAL` o bundle fica pequeno PORQUE os bytes não foram copiados — eles seguem obrigatórios no checkpoint de origem. Medido no Muse-Glimmer-30B (IQ2_XXS, 10,73 GB, 731 tensores, 688 externos): o registro dizia `disk_reduction_pct: 85,51` e `ram_reduction_pct: 85,52` quando a redução REAL é **6,5%** (bundle 1,56 GB + externo 8,48 GB = 10,03 GB exigidos). Agora `candidate_disk_bytes` é o disco EXIGIDO (`bundle + external_source_bytes`), `ram_reduction_pct` e `estimated_candidate_bytes` saem de `all_in_ram_bytes`, o console deixa de imprimir bundle-vs-fonte como "Disk reduction" quando há external, e o card "Modelos convertidos" **ordena por `all_in_ram_bytes` ascendente** com "TOTAL em RAM" como primeira métrica, selo "depende da fonte" e linha "Fora do bundle (na fonte)". `cascade_bundle_directory_bytes` e `bundle_disk_reduction_pct` seguem no manifesto como detalhe, nunca como headline.
- **`--input` apontando para o cache do Hugging Face morria com `Header truncado`** (contrato §29.8): `convert()` faz `Path(args.input).resolve()` e `resolve()` **segue symlink** — em `.../snapshots/<rev>/model.gguf → blobs/<sha256>` o alvo NÃO tem extensão, então o despacho por sufixo mandava o GGUF para o `SafeTensorSource`. Agora `sniff_container()` decide pelos **magic bytes** (`GGUF` → gguf, `PK` → npz, ausência de magic → safetensors) e o sufixo é apenas fallback, para que um `.gguf` vazio ou truncado ainda falhe COMO GGUF. Dispensa o hardlink de contorno: `--input` aceita o caminho do cache do HF direto. A guarda destrutiva de `--delete-source-shards` mantém de propósito o teste por sufixo `.safetensors` — precisa falhar fechada e preservar um blob sem extensão. Regra permanente: **nenhuma decisão de formato depende do NOME do arquivo**.
