...
- **Motor de execução CASCADE v2** (`core/cascade/runtime_v2/`, contrato §30): F0 em kernel C fundido (`q4k_gemv`) que **desquantiza dentro do produto e nunca materializa W**, F1 low-rank com V transposto para `(rank, in)` e leitura contígua com FMA, pack/unpack vetorizado do super-bloco de 144 B (o mesmo layout de `mmap_bundle.hpp`) e gate calibrado offline. Sem a lib compilada o módulo **recusa carregar** em vez de cair em fallback lento silencioso. Medido em stack sintético (4 threads, AVX2): **37,9 tok/s @ 0,28 GB** contra 21,8 tok/s @ 2,01 GB do caminho antigo (**7,18× a RAM**) e 0,87 tok/s do `CASCADE_LOW_MEM=1` (**43,6×**); kernel F0 isolado a 16,55 GB/s, 92% do llama.cpp Q4_K.
- **Registro `P1_CASCADE_RUNTIME_V2_KERNEL`** no histórico (protocolo `RUNTIME_KERNEL_SYNTHETIC_V1`, grupo próprio "P1 · Motor de execução"): como a regressão é sobre **stack sintético e não `model.generate`**, `baseline_tok_s`/`candidate_tok_s`/`*_ram_bytes` de topo são `null` por design e os números medidos ficam em `metrics.runtime` com `scope: "SYNTHETIC_STACK"` — aquele tok/s não é comparável com as baterias E2E e não entra em ranking.

### :pushpin: Fixes

- **O Confidence Gate nunca filtrou nada em decode** (contrato §30.1): `decide_gate` tirava o threshold de `torch.quantile` sobre o **próprio batch**; com batch 1 o quantil É o score e a máscara virava `score >= score`, sempre True — o F1 rodava em **100% dos tokens de decode**. Medido no harness, a taxa era função apenas do tamanho do batch, idêntica em 8 sorteios por tamanho (batch 1 → 1,000 · 2 → 0,500 · 4 → 0,250 · 8 → 0,375 · 16 → 0,3125): o gate media o tamanho do batch, não a confiança da ativação. v1 calibra offline (`GateCalibrator.observe/freeze` → `GateConfig.fixed_threshold`, para gravar no bundle) e, sem threshold em batch pequeno, escolhe **F0_ONLY com `warning` na telemetria** em vez de reportar uma taxa inventada. Nenhum registro publicado carregava taxa de gate CASCADE, então o histórico não foi contaminado — o bug era latente para o próximo run de `c3_methodology_auto_batteries.py` e `final_phase_auto_batteries.py`.
- **Cache FP32 anulava o formato de 4,5 bpw** (§30.2): `block_runtime.py` mantinha `_w0_cache` com W desquantizado em fp32 — 32 bpw residentes. Medido: 2,01 GB contra 0,28 GB.
- **"Vetorização" AVX2 que era gather escalar** (§30.3): `cpp/avx2_lowrank.cpp` montava o vetor com `_mm256_set_ps` a partir de 8 cargas escalares em stride `rank`, e tinha um loop cujo corpo calculava `acc` e `Vr` sem usar nenhum dos dois. Loop morto removido e o arquivo anotado como supersedido pelo kernel do v2.
- **F1 não se paga — terceira medição independente** (§30.5): cosseno 0,996689 com F0 contra 0,996719 com F0+F1, **+3e-5** por um GEMV extra. Some-se ao piso derivado do gate (§29.4) e aos 0,064 de energia capturada em BF16 (§29.11): o resíduo de quantização é ruído de posto alto e rank ≤ 32 não o resgata. Enquanto isso valer, o formato efetivo do CASCADE é INT4 groupwise puro.
