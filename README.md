...
- **Conversor v2 (`CASCADE-Q4K/2.0`)** em `core/cascade/runtime_v2/convert.py` (contrato §31): grava **direto no formato do executor** (138 B em g=64, 144 B em g=32 por super-bloco) — zero repack na carga, que era a maior perda do v1. Escada `q4k/g64+clip → q4k/g32+clip` **sem fallback raw** para 2D: na amostra, 34 tensores em 4,3125 bpw, 2 em 4,5 e **zero raw** (o `o_proj/L51` que o v1 mandava a 16 bpw passou a 4,31). Embeddings e `lm_head` passam a ser quantizados com cosseno **medido** (0,996976 / 0,996913) em vez do raw 16 bpw — é daí que vem a maior parte da economia. Medido na amostra do Muse-Glimmer-30B BF16: **2,90 GB → 0,78 GB (73,1%) em 99,7 s**, contra 69,35% em 198,7 s do v1 (2× mais rápido, 3,7 pp menor), ao custo de 7,3× no pico de RSS de conversão (0,43 → 3,14 GiB, ainda cabe em 8 GB). Fonte BF16/F16/F32 apenas — recomprimir GGUF segue sendo beco sem saída medido.
- **Suporte a g=64 no executor**: `q4k_pack.sup_bytes_for`, `q4k_gemv_i8_g` e `Q4KLinearModule(g=...)` passam a tratar super-bloco de 138 B (4 sub-escalas) além do de 144 B (8), com o mesmo inner loop.
- **Registro no histórico com a MARGEM do veredito** (§31.6): o "cabe na máquina de 24 GB" do 30B vem de uma projeção ×52 de ponto único com folga de **0,46 GiB**, e o ponto de ruptura é **+3,24%** de erro na projeção. O card passa a exibir "Folga em 24 GB" com selo PROJETADO e a nota do ponto de ruptura — um "cabe" por 0,46 GiB não é a mesma afirmação que um "cabe" por 8,46 GiB. As comparações de tok/s do relatório externo (−3,3% vs Q4_K_XL, +96,2% vs IQ2_XXS, >10× na máquina de 24 GB) derivam de razão de banda e de modelagem de paginação, não de `model.generate`, e por isso **não entram no registro**.

### :pushpin: Fixes

- **`residency_report` subtraía 8 GiB duas vezes** (§31.5): `MACHINE_CLASSES_GIB = (8,16,24,40)` já eram orçamentos, mas a função ainda fazia `budget = cls - 8` e rotulava a linha como `maquina_{cls+8}gb`. A linha rotulada "maquina_24gb" recebia orçamento de **8 GiB** e imprimia **NÃO CABE** para um bundle de 15,54 GiB que **CABE** em 24 GB pela regra do §28 — ou seja, o código contradizia a manchete do próprio teste, e o relatório externo que afirmava o correto não pode ter saído desta função. Corrigido para `MACHINE_TOTAL_GIB = (16,24,32,48)` com `budget = max(total-8, total/2)`, agora com `folga_gib` e `regra_orcamento` explícitos no manifesto.
- **Mudança de política de gate declarada** (§31.4): sem fallback raw, um tensor que reprova todos os degraus é gravado **assim mesmo** (`RESCUE_LAST_RUNG`) com `quality_flag`. Isso **rompe o invariante do §29.5** ("quando nada passa, o resultado continua sendo passthrough exato") — de propósito, trocando 16 bpw exatos por 4,5 bpw com perda declarada. O resumo do manifesto passou a expor `all_tensors_passed_gate`, `below_gate_tensor_count` e `below_gate_tensors`, e o console imprime um ATENÇÃO com os nomes: o bundle deixou de ser aprovado por construção.
- **Docstring prometia escada que o código não tem**: dizia `q4k/g64 → q4k/g32 → q5k → q6k` com "pior caso 6,5 bpw" e embeddings em `q6k/g32`, mas `LADDER` tem apenas dois degraus de 4 bits e os embeddings vão para `q4k/g32`. Os degraus 5/6-bit dependem de kernel que não existe; `LADDER` é a fonte da verdade.
